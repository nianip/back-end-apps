...
if ($method === 'GET') {
    ...
    if (!empty($input['nama_lapangan']) && !empty($input['alamat']) && !empty($input['deskripsi']) && !empty($input['ukuran']) && !empty($input['kapasitas']) && !empty($input['harga'])) {
        ...
        $sql = "INSERT INTO tempat (nama_lapangan, alamat, deskripsi, ukuran_lapangan, kapasitas_lapangan, harga) 
                VALUES ('$nama', '$alamat', '$deskripsi', '$ukuran', $kapasitas, $harga)";

        if ($conn->query($sql) === TRUE) {
            echo json_encode(["success" => true, "message" => "Field successfully added."]);
        } else {
            http_response_code(500);
            echo json_encode(["error" => "Failed to add field: " . $conn->error]);
        }
    } else {
